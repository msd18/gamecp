...
class PHPMailer
{
    ...
    public $PluginDir = "./includes/main/";
    public $Version = "2.2";
    ...
    public function SendmailSend($header, $body)
    {
        ...
        fputs($mail, $body);
        $result = pclose($mail);
        if( version_compare(phpversion(), "4.2.3") == 0 - 1 ) 
        {
            $result = $result >> 8 & 255;
        }
    ...
    public function SmtpConnect()
    {
        ...
        $index = 0;
        for( $connection = $this->smtp->Connected(); $index < count($hosts) && $connection == false; $index++ ) 
        {
            $hostinfo = array(  );
    ...
    public function CreateHeader()
    {
        ...
        if( ($this->Mailer == "sendmail" || $this->Mailer == "mail") && 0 < count($this->cc) ) 
        {
            $result .= $this->AddrAppend("Cc", $this->cc);
        }
        ...
        if( isset($images[2]) ) 
        {
            foreach( $images[2] as $i => $url ) 
            {
                if( !preg_match("/^[A-z][A-z]*:\\/\\//", $url) ) 
                {
                    ...
                    if( 1 < strlen($directory) && substr($basedir, 0 - 1) != "/" ) 
                    {
                        $directory .= "/";
                    }
