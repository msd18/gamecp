...
if( !empty($setmodules) ) 
{
    ...
    $module["Donations"]["Purchase GP (PayPal)"] = $file;
}
else
{
    $lefttitle = "Purchase Game Points (PayPal)";
        ...
        if( isset($isuser) && $isuser == true ) 
        {
            ...
            if( $exit_stage == 0 ) 
            {
                ...
                for( $i = 1; $i < count($c_price); $i++ ) 
                {
                    ...
                    $out .= "\t\t<td class=\"" . $bgcolor . "\" align=\"center\"><b>" . number_format($c_total[$i]) . "</td>" . "\n";
                    $out .= "\t\t<td class=\"" . $bgcolor . "\" align=\"center\"><form action=\"https://www.paypal.com/cgi-bin/webscr\" method=\"post\">" . game_cp_9(number_format($c_price[$i], 2, ".", ""), $c_total[$i], $custom) . "</form></td>" . "\n";
                    $out .= "\t</tr>" . "\n";
                }
                $out .= "\t</table>";
                return 1;
